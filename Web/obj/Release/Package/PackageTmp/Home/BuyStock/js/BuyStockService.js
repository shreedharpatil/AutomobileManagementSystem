    ...
    var model = function (http, rootScope, warningMessageService, captureImageService) {
        ...
        self.CalculateTotalTransactionAmount = function () {
            ...
            _.each(self.Goods, function (good) {
                self.DealerTransaction.TotalAmount += isNaN(good.TotalAmount) ? 0 : parseFloat(good.TotalAmount);
            });
        };
        ...
        self.DeductPaidAmount = function () {
            ...
            var total = isNaN(self.DealerTransaction.TotalAmount) ? 0 : parseFloat(self.DealerTransaction.TotalAmount);
            self.DealerTransaction.DueAmount = total - paid;
        };
        ...
        self.RemoveGood = function (good) {
            ...
            var total = isNaN(self.DealerTransaction.TotalAmount) ? 0 : parseFloat(self.DealerTransaction.TotalAmount);
            self.DealerTransaction.TotalAmount = total - goodtotal;
            var paid = isNaN(self.DealerTransaction.PaidAmount) ? 0 : parseFloat(self.DealerTransaction.PaidAmount);
            if (paid > self.DealerTransaction.TotalAmount) {
                self.DealerTransaction.PaidAmount = 0;
                self.DealerTransaction.DueAmount = 0;
            }
            else {
                self.DealerTransaction.DueAmount = self.DealerTransaction.TotalAmount - paid;
            }
